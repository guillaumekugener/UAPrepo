...
var springTransitionable = new Transitionable(springRestLength);
...
context.add(leftMajorWallModifier).add(leftMajorWallSurface);
context.add(movingSpringModifier).add(movingSpringSurface);
context.add(massModifier).add(massSurface);
...
*   1. Create a wall surface/constraint on the right and a wall constraint on the left (linked to the)
*   mass that both apply some force on the spring particle, forcing it to contract.
*   2. Change the size of the spring surface as the mass is moving. 
*   3. Using a transitionable
*/ 
movingSpringModifier.sizeFrom(changeSpringLength);
function changeSpringLength() {
    return [massSurface.particle.getPosition()[0] + springRestLength, 20];
}
